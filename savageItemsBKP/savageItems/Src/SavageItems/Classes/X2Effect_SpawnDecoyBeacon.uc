class X2Effect_SpawnDecoyBeacon extends X2Effect_SpawnUnit;
...
function vector GetSpawnLocation(const out EffectAppliedData ApplyEffectParameters, XComGameState NewGameState)
{
	...
	{
		`Redscreen("Attempting to create X2Effect_SpawnDecoyBeacon without a target location! @dslonneger");
		return vect(0,0,0);
	}
...
function OnSpawnComplete(const out EffectAppliedData ApplyEffectParameters, StateObjectReference NewUnitRef, XComGameState NewGameState, XComGameState_Effect NewEffectState)
{
	...
	local XComGameState_Unit TurretGameState;
    ...
    TurretGameState = XComGameState_Unit(NewGameState.GetGameStateForObjectID(NewUnitRef.ObjectID));
    TurretGameState.kAppearance.bGhostPawn = true;
