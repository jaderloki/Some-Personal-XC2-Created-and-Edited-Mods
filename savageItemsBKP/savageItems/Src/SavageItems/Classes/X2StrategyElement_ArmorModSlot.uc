...
static function bool HasAmodSlot(CHItemSlot Slot, XComGameState_Unit UnitState, out string LockedReason, optional XComGameState CheckGameState)
{
	...
	if (default.AbilityUnlocksAModSlot.Length == 0)
	{
		...
		DefaultCanHaveSlot = UnitState.IsSoldier() && !UnitState.IsRobotic();//this bit is added and im going with no amods on robots sice it can cause wierdness I can't be bothered to iron out right now.
	}
