...
//---------------------------------------------------------------------------------------
class SeqAct_GetAllCovertOps extends SequenceAction;
...
event Activated()
{
	...
	foreach Units(UnitToFilter)
	{
		if(UnitToFilter!= none)
		{
			...
			{
				if(UnitToFilter.GetUnitValue('IsCovertOp', SoldierValue))
				{
					if(SoldierValue.fValue > 0)
					{
					`Log("SeqAct_GetAllCovertOps: Found unit "@ UnitToFilter);
	...
	if(!bUnitsWereFound)
	{
		`Redscreen("SeqAct_GetAllCovertOps: Could not find any units on the designated team.  Check for proper usage of this node.");		
	}
}
...
{
	ObjCategory="Unit"
	ObjName="Get All Covert Ops On Team"
