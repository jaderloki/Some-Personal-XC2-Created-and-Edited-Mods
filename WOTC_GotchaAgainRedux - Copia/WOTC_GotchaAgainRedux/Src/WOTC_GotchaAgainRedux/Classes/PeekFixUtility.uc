...
static function MakeConcealmentMarkers(XComPathingPawn PathingPawn, out array<TTile> ConcealmentMarkers)
{
	...
	local int graceTilesTMP, HighestTargetCoverPoint;
	...
	RevealedEnemies = GetRevealedSpottersOfUnit(ActiveUnitState, `GETMCMVAR(bAlwaysShowPodActivation));
		...
		foreach RevealedEnemies(EnemyUnit)
		{
			...
			*/
			...
			)
			{
				...
				if (
					`GETMCMVAR(HideWithHeightAdvantage) && 
				...
				if (
					...
					`LOG("Visible n_ " $ TargetCover);
					if (
						`GETMCMVAR(HideWithHeightAdvantage) && 
						Tile.Z >= (EnemyUnit.TileLocation.Z + class'X2TacticalGameRuleset'.default.UnitHeightAdvantage)
					){
						if(TargetCover == CT_None){
							HighestTargetCoverPoint = (HighestTargetCoverPoint > 2) ? HighestTargetCoverPoint : 2;
						}else if(TargetCover == CT_MidLevel){
							HighestTargetCoverPoint = (HighestTargetCoverPoint > 1) ? HighestTargetCoverPoint : 1;
						}
					}else{
						if(TargetCover == CT_None){
							HighestTargetCoverPoint = (HighestTargetCoverPoint > 4) ? HighestTargetCoverPoint : 4;
						}else if(TargetCover == CT_MidLevel){
							HighestTargetCoverPoint = (HighestTargetCoverPoint > 2) ? HighestTargetCoverPoint : 2;
						}
					}
